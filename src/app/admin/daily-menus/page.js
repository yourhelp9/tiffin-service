...
const DailyMenusPage = () => {
    ...
    const fetchDailyMenus = async (storedToken) => {
        try {
            const response = await fetch('http://127.0.0.1:8000/api/admin/daily-menus', {
                headers: {
    ...
    const handleFormSubmit = async (e) => {
        ...
        const url = editingMenu 
            ? `http://127.0.0.1:8000/api/daily-menus/${editingMenu.id}`
            : 'http://127.0.0.1:8000/api/daily-menus';
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:8000/api/daily-menus/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Accept': 'application/json'
