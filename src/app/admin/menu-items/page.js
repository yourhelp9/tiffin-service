...
const MenuItemsPage = () => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        if (editingItem) {
            url = `http://127.0.0.1:8000/api/menu-items/${editingItem.id}`;
                        ...
                        {menuItems.map((item) => (
                            <tr key={item.id}>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    {item.image_url ? (
                                        <img src={`http://127.0.0.1:8000/storage/${item.image_url}`} alt={item.name} className="h-10 w-10 rounded-full" />
                                    ) : (
