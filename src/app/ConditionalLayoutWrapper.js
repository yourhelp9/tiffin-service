...
export default function ConditionalLayoutWrapper({ children }) {
  ...
  return (
    <>
      {/* Header */}
      <header className="bg-white shadow-md h-16 fixed top-0 left-0 w-full z-20">
        <div className="w-full px-4 sm:px-6 lg:px-20 flex justify-between items-center h-full">
          {/* Logo */}
          <Link href="/">
        ...
        {!isLoggedIn && menuOpen && (
          <div className="sm:hidden bg-white shadow-md px-4 py-3 space-y-2">
            {nonLoggedInLinks.map((link) => (
              <Link key={link.name} href={link.href}>
                <span
                  className={`block text-gray-700 font-medium py-2 px-2 rounded transition
                    ${isActive(link.href) ? 'text-[#63ab45] bg-gray-100' : 'hover:text-[#63ab45]'}`}
      ...
      {/* Main Content */}
      <main className={`mt-16 ${isLoggedIn ? 'pb-20' : ''}`}>{children}</main>
        ...
        <nav className="fixed bottom-0 left-0 w-full bg-white shadow-lg z-10">
          <div className="max-w-5xl mx-auto flex justify-around items-center h-16 text-gray-600">
            {loggedInLinks.map((link) => (
              <Link key={link.name} href={link.href}>
                <span
                  className={`flex flex-col items-center cursor-pointer transition-colors duration-200 hover:text-[#4e8737]
