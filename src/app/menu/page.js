...
const MenuPage = () => {
    ...
    const fetchAllData = async () => {
        ...
        try {
            ...
            const userSelectionsResponse = await axios.get('http://127.0.0.1:8000/api/user-selections', {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            setUserSelections(userSelectionsResponse.data);

            const menuResponse = await axios.get('http://127.0.0.1:8000/api/daily-menus');
    ...
    const handleSaveSelections = async () => {
        ...
        try {
            const response = await axios.post('http://127.0.0.1:8000/api/user-selections', selectionData, {
                headers: {
                        ...
                        <div key={dailyMenu.id} className="space-y-4">
                            ...
                            >
                                {dailyMenu.option1?.image_url ? (
                                    <img 
                                        src={`http://127.0.0.1:8000/storage/${dailyMenu.option1.image_url}`} 
                            ...
                            {dailyMenu.option2?.name && (
                                ...
                                >
                                    {dailyMenu.option2?.image_url ? (
                                        <img 
                                            src={`http://127.0.0.1:8000/storage/${dailyMenu.option2.image_url}`} 
