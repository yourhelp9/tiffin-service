...
const ProfilePage = () => {
    ...
    const fetchUser = async (token) => {
        try {
            ...
            setProfileImagePreview(response.data.profile_image_url ? `http://127.0.0.1:8000/storage/${response.data.profile_image_url}` : null);
        } catch (error) {
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            setProfileImagePreview(response.data.user.profile_image_url ? `http://127.0.0.1:8000/storage/${response.data.user.profile_image_url}` : null);
            setIsEditing(false);
                    ...
                    <form onSubmit={handleUpdate} className="space-y-5 mt-8">
                        ...
                        <div className="flex flex-col sm:flex-row justify-between space-y-4 sm:space-y-0 sm:space-x-4 mt-6">
                            <button 
                                ...
                                onClick={() => {
                                    setIsEditing(false);
                                    setProfileImagePreview(user.profile_image_url ? `http://127.0.0.1:8000/storage/${user.profile_image_url}` : null);
                                    setMessage('');
                                }} 
